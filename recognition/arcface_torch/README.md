...
<!-- “arcface_torch”存储库是ArcFace算法的官方实现。它支持使用多个分布式训练示例进行分布式和稀疏训练，包括混合精确训练和梯度检查点等几种节省内存的技术。它还支持ViT模型和数据集的培训，包括WebFace42M和Glint360K，这是两个最大的开源数据集。此外，该存储库还附带了一个用于转换为ONNX格式的内置工具，使其易于提交到MFR评估系统。 -->
...
<!-- 要训练模型，请执行' train.py '脚本，其中包含配置文件的路径。下面提供的示例命令演示了执行分布式训练的过程。 -->
### 1. To run on one GPU:
...
<!-- 不建议使用单个GPU进行训练，因为这可能会导致更长的训练时间和次优性能。为了获得最佳效果，我们建议使用多个GPU或一个GPU集群。 -->
