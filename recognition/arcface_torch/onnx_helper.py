...
# 从命令行参数解析工作目录和比赛类别，创建 ArcFaceORT 对象，并对其进行检查。然后，将检查结果打印到标准输出流中。
if __name__ == '__main__':
    ...
    # 创建了一个 ArcFaceORT 类的对象 handler，并将 workdir 参数传递给构造函数
    handler = ArcFaceORT(args.workdir)
    # 调用 handler 对象的 check 方法，并将 track 参数传递给它，将结果存储在 err 变量中
